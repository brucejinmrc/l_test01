...
public class TestEnums {
	 ...
	 * 
	 * If no constructor is added, then the usual default constructor is created
	 * by the system, and declarations of the enum constants will correspond to
	 * calling this default constructor.
	 */
	public enum Lepton {
		// each constant implicity calls a constructor :
		ELECTRON(-1, 1.0E-31), NEUTRINO(0, 0.0);

		/*
		 * This constructor is private. Legal to declare a non-private
		 * constructor, but not legal to use such a constructor outside the
		 * enum. Can never use "new" with any enum, even inside the enum class
		 * itself.
		 */
	 ...
	 * 
	 * Here, "static" may be left out, since enum types which are class members
	 * are implicitly static.
	 */
	static enum Direction {
		NORTH, SOUTH, EAST, WEST; // note semicolon needed only when extending
									// behavior
		...
		public String toString() {
			/*
			 * Either name() or super.toString() may be called here. name() is
			 * final, and always returns the exact name as specified in
			 * declaration; toString() is not final, and is intended for
			 * presentation to the user. It seems best to call name() here.
			 */
	 ...
	 */
	private enum Flavor {
		CHOCOLATE(100), VANILLA(120), STRAWBERRY(80);
	...
	private static void exerEnumsMiscellaneous() {
		...
		// compareTo order is defined by order of appearance in the definition
		// of  the enum
		...
		Lepton pp = Lepton.ELECTRON;
		Lepton oo = Lepton.ELECTRON;
		System.out.println("pp==oo? " + (pp == oo));
		System.out.println("pp-harge? " + pp.getCharge());
	...
	private static void exerMutableEnum() {
		Flavor.VANILLA.setCalories(75); // change the state of the enum
										// "constant"
		log("Calories in Vanilla: " + Flavor.VANILLA.getCalories());
	}

	private static void exerEnumRange() {
		for (Direction direction : EnumSet.range(Direction.NORTH,
				Direction.SOUTH)) {
	...
	/**
	 * The valueOf method uses name(), not toString(). There is no need to
	 * synchronize valueOf with toString.
	 */
